...
class FunctionalApi {
  ...
  async getBlockStatus(userId: number) {
    const me = await this.appUser();
    if (Number(me.id) === Number(userId)) return false;
    const result = await insforge.database.from('blocks').select('id').eq('blocker_id', me.id).eq('blocked_id', userId).maybeSingle();
    if (result.error) fail(result.error, 'Unable to check block status');
  ...
  async blockUser(userId: number) {
    ...
    if (!targetId || targetId === Number(me.id)) throw new Error('You cannot block yourself.');
    const existing = await insforge.database.from('blocks').select('id').eq('blocker_id', me.id).eq('blocked_id', targetId).maybeSingle();
    if (existing.error) fail(existing.error, 'Unable to check block status');
    if (!existing.data) {
      const created = await insforge.database.from('blocks').insert({ blocker_id: me.id, blocked_id: targetId }).select('*').single();
      if (created.error) fail(created.error, 'Unable to block member');
    }
    await insforge.database.from('connections').delete().or(`and(user_id.eq.${me.id},connected_user_id.eq.${targetId}),and(user_id.eq.${targetId},connected_user_id.eq.${me.id})`);
    return { blocked: true };
  }
  ...
  async unblockUser(userId: number) {
    const me = await this.appUser();
    const targetId = Number(userId);
    const result = await insforge.database.from('blocks').delete().eq('blocker_id', me.id).eq('blocked_id', targetId);
    if (result.error) fail(result.error, 'Unable to unblock member');
