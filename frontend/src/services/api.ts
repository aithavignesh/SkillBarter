...
class ApiClient {
  ...
  async logout() {
    const phone = sessionStorage.getItem('skillbarter_phone');
    if (phone) {
      // Phone OTP sessions are access-token based. Avoid signOut(), which can
      // invoke InsForge's refresh/CSRF flow even though no browser refresh
      // cookie exists for this session.
      this.clearToken();
  ...
  async getFeed(type?: string) {
    ...
    const authorsResult = await insforge.database
      .from('users')
      .select('id,full_name,avatar_url,headline,latitude,longitude,location_visibility,trust_score,verified,is_active,badges')
      .in('id', authorIds);
