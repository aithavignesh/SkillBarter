  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-px border border-[#e1e4e8] bg-[#e1e4e8] sm:grid-cols-3">
        ...
        <div className="bg-white p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">2. Plan</p><p className="mt-1 text-xs font-semibold text-[#17233b]">Agree on time and format</p><p className="mt-1 text-[10px] text-slate-500">Use chat to settle the session details.</p></div>
        <div className="bg-white p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">3. Learn</p><p className="mt-1 text-xs font-semibold text-[#17233b]">Complete the session</p><p className="mt-1 text-[10px] text-slate-500">Finish the exchange and leave a peer review.</p></div>
      </div>
      {loading ? (
            ...
            {exchanges.map((ex) => {
              ...
              return (
                <div key={ex.id} className="grid gap-4 px-5 py-5 md:grid-cols-[minmax(230px,1.2fr)_minmax(220px,1fr)_180px_190px] md:items-center">
                  ...
                  <div><Badge variant={statusColors[ex.status] || 'slate'} size="md">{ex.status === 'PENDING' && !isRequester ? 'Needs your response' : ex.status === 'ACTIVE' ? 'Ready to learn' : ex.status}</Badge><p className="mt-1 text-[10px] text-slate-400">{new Date(ex.created_at).toLocaleDateString()}</p></div>
                  <div className="flex flex-wrap gap-2 md:justify-end">
                    {ex.status === 'PENDING' && !isRequester && <><Button size="sm" variant="outline" onClick={() => action(()=>api.rejectExchange(ex.id))}>Decline</Button><Button size="sm" onClick={() => action(()=>api.acceptExchange(ex.id))}>Accept & Start</Button></>}
                    ...
                    {ex.status === 'ACTIVE' && !myCompleted && <Button size="sm" onClick={() => action(async()=>{ const updated=await api.completeExchange(ex.id); if(updated.status==='COMPLETED'){setReviewExchange(updated);setIsReviewOpen(true);} })} icon={<CheckCircle2 className="h-3.5 w-3.5" />}>Mark Session Complete</Button>}
                    {ex.status === 'ACTIVE' && <><Link to={`/messages/${partner.id}`}><Button size="sm" variant="outline" icon={<MessageSquare className="h-3.5 w-3.5" />}>Chat</Button></Link><Button size="sm" variant="ghost" onClick={() => { const reason = window.prompt('Why are you cancelling this exchange?'); if (reason?.trim()) void action(()=>api.cancelExchange(ex.id, reason.trim())); }}>Cancel</Button></>}
