...
import { ArrowLeftRight, Repeat, CheckCircle2, Calendar, MessageSquare, Star, Sparkles, MapPin, Clock, ArrowUpRight } from 'lucide-react';
...
export const ExchangesPage: React.FC = () => {
  ...
  const action = async (fn: () => Promise<any>) => { try { await fn(); await loadExchanges(); } catch (e: any) { alert(e?.message || 'Action failed'); } };
  const tabs = [
  ...
  return (
    ...
    >
      ...
      {loading ? <div className="mt-5 border border-[#e1e4e8] bg-white py-20 text-center text-sm text-slate-400">Loading exchange workspace…</div> : exchanges.length === 0 ? <div className="mt-5 border border-[#e1e4e8] bg-white py-20 text-center"><Repeat className="mx-auto h-9 w-9 text-slate-300" /><h3 className="mt-3 text-sm font-bold text-[#17233b]">No exchanges in this view</h3><p className="mt-1 text-xs text-slate-500">Start with a smart match and send your first barter proposal.</p><Link to="/discover" className="mt-4 inline-block"><Button size="sm">Browse skills</Button></Link></div> : <div className="mt-5 overflow-hidden border border-[#e1e4e8] bg-white">
        <div className="hidden grid-cols-[minmax(230px,1.2fr)_minmax(220px,1fr)_180px_150px] border-b border-[#e1e4e8] bg-[#f7f8f7] px-5 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-400 md:grid"><span>Partner / exchange</span><span>Details</span><span>Status</span><span className="text-right">Actions</span></div>
        <div className="divide-y divide-[#e1e4e8]">
          {exchanges.map((ex) => {
            ...
            return <div key={ex.id} className="grid gap-4 px-5 py-5 md:grid-cols-[minmax(230px,1.2fr)_minmax(220px,1fr)_180px_150px] md:items-center">
              ...
              <div className="space-y-1 text-[11px] text-slate-500"><p className="line-clamp-2 italic">“{ex.proposal_message}”</p><p className="flex flex-wrap gap-3"><span><Calendar className="mr-1 inline h-3 w-3" />{ex.preferred_date || 'Flexible'}</span><span><Clock className="mr-1 inline h-3 w-3" />~{ex.estimated_hours}h</span></p></div>
              ...
              <div className="flex flex-wrap gap-2 md:justify-end">
                ...
                {ex.status === 'ACTIVE' && <Link to="/messages"><Button size="sm" variant="outline" icon={<MessageSquare className="h-3.5 w-3.5" />}>Chat</Button></Link>}
      ...
      {reviewExchange && <ExchangeReviewModal isOpen={isReviewOpen} onClose={()=>setIsReviewOpen(false)} exchange={reviewExchange} onSuccess={()=>{setIsReviewOpen(false);alert('Review submitted!');loadExchanges();}} />}
    </AppPageShell>
  );
};
