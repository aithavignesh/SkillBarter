              ...
              return (
                <div key={ex.id} className="grid gap-4 px-5 py-5 md:grid-cols-[minmax(230px,1.2fr)_minmax(220px,1fr)_180px_190px] md:items-center">
                  ...
                  <div className="flex flex-wrap gap-2 md:justify-end">
                    ...
                    {ex.status === 'PENDING' && isRequester && <><span className="text-[10px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1">Waiting for partner</span><Button size="sm" variant="outline" onClick={() => action(()=>api.cancelExchange(ex.id, 'Request withdrawn by requester'))} icon={<XCircle className="h-3.5 w-3.5" />}>Withdraw</Button></>}
