    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[1.5fr_1fr]">
      <Card className="overflow-hidden p-0">
        <div className="flex items-center justify-between border-b border-[#e1e4e8] bg-[#f7f8f7] px-5 py-4">
          <div><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#d31d24]">Community feed</div><h2 className="mt-1 text-sm font-bold text-[#17233b]">What members are sharing</h2></div>
          <Link to="/feed"><Button size="sm" variant="outline">Open feed</Button></Link>
        </div>
        {postLoading ? <div className="p-8 text-center text-xs text-slate-400">Loading community activity…</div> : posts.length ? <div>{posts.slice(0,5).map((p:any)=><div key={p.id} className="border-b border-[#edf0f2] p-5 last:border-0"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-[#d31d24]">{p.post_type === 'WORKSHOP' ? <CalendarDays className="h-3.5 w-3.5"/> : <MessageSquare className="h-3.5 w-3.5"/>}{p.post_type || 'COMMUNITY'}</div><h3 className="mt-1 text-sm font-bold text-[#17233b]">{p.title || 'Community update'}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">{p.content}</p><div className="mt-3 flex items-center gap-4 text-[10px] text-slate-400"><span className="inline-flex items-center gap-1"><Heart className="h-3 w-3"/> {p.likes_count || 0}</span><span>{p.created_at ? new Date(p.created_at).toLocaleDateString() : ''}</span></div></div>)}</div> : <div className="p-8 text-center text-xs text-slate-400">No community posts yet.</div>}
      </Card>
      <Card className="p-5">
        <div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-[#d31d24]"/><h2 className="text-sm font-bold text-[#17233b]">Community actions</h2></div>
        <div className="mt-4 space-y-2">
          <Link to="/discover" className="flex items-center justify-between border border-[#e1e4e8] bg-white px-4 py-3 text-xs font-semibold text-[#17233b] hover:bg-[#f7f8f7]"><span>Discover members</span><ArrowRight className="h-3.5 w-3.5 text-[#d31d24]"/></Link>
          <Link to="/matches" className="flex items-center justify-between border border-[#e1e4e8] bg-white px-4 py-3 text-xs font-semibold text-[#17233b] hover:bg-[#f7f8f7]"><span>Find skill matches</span><ArrowRight className="h-3.5 w-3.5 text-[#d31d24]"/></Link>
          <Link to="/workshops" className="flex items-center justify-between border border-[#e1e4e8] bg-white px-4 py-3 text-xs font-semibold text-[#17233b] hover:bg-[#f7f8f7]"><span>Browse workshops</span><ArrowRight className="h-3.5 w-3.5 text-[#d31d24]"/></Link>
        </div>
      </Card>
    </div>
    </div>
