...
export const MonetizationPage: React.FC = () => {
  ...
  useEffect(() => { if (userId) { setState(getMonetizationState(userId)); void hydrateMonetizationState(userId).then(setState); } }, [userId]);
  ...
  const boostProfile = () => {
    notify('Profile boosts are not enabled until credits are enforced server-side.');
  };
  const togglePremium = async () => {
    if (state.premium) {
      patch({ premium: false, premiumUntil: null, priorityMatching: false }, 'Premium access disabled.');
      return;
    }
    try {
      ...
      const orderResponse = await fetch('/api/payments/create-order', {
        ...
        body: JSON.stringify({ plan: 'premium_monthly', email: currentUser?.email }),
      });
      ...
      if (!document.getElementById(scriptId)) {
        await new Promise<void>((resolve, reject) => {
          ...
          script.onerror = () => reject(new Error('Unable to load the Razorpay checkout.'));
      ...
      await new Promise<void>((resolve, reject) => {
        const checkout = new Razorpay({
          ...
          prefill: { email: currentUser?.email || order.prefill?.email, name: currentUser?.full_name || '' },
          theme: { color: '#d31d24' },
          handler: async (response: any) => {
            try {
              const verifyToken = localStorage.getItem('skillbarter_token');
              const verification = await fetch('/api/payments/verify-payment', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', ...(verifyToken ? { Authorization: `Bearer ${verifyToken}` } : {}) },
                body: JSON.stringify(response),
              });
              const result = await verification.json();
              if (!verification.ok || !result.success) throw new Error(result.error || 'Payment verification failed.');
              const next = await hydrateMonetizationState(userId);
              setState(next);
              setUsageTick(v => v + 1);
              notify('Payment successful. Premium Membership is now active for 1 month.');
              resolve();
      ...
      notify(error instanceof Error ? error.message : 'Unable to complete payment.');
    } finally {
  ...
  const requestVerification = () => {
    ...
    notify('Verification requests will be enabled with the server-side review workflow.');
  };
