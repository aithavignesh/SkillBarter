...
export const MonetizationPage: React.FC = () => {
  ...
  const boostProfile = () => {
    ...
    try {
      const next = recordBoost(userId, 7);
      setState(next);
      setUsageTick(v => v + 1);
      notify('Profile featured for 7 days. 50 skill credits were used.');
    } catch (error) {
      notify(error instanceof Error ? error.message : 'Unable to boost profile.');
    }
  };
  const togglePremium = () => {
    if (state.premium) {
      patch({ premium: false, premiumUntil: null, priorityMatching: false }, 'Premium membership cancelled. Current access remains off for product testing.');
    } else {
      const next = activatePremium(userId, 1);
      setState(next);
      setUsageTick(v => v + 1);
      notify('Premium activated for 1 month. Priority matching is now enabled.');
    }
  };
      ...
      <div className="mx-auto w-full max-w-[1320px]">
        ...
        <section className="border-b border-[#e1e4e8] pb-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div className="grid grid-cols-3 gap-2"><div className="min-w-[95px] border border-[#e1e4e8] bg-white px-4 py-3"><p className="text-[10px] text-[#697386]">Skill Credits</p><p className="mt-1 text-xl font-extrabold text-[#17233b]">{state.credits}</p></div><div className="min-w-[95px] border border-[#e1e4e8] bg-white px-4 py-3"><p className="text-[10px] text-[#697386]">Premium</p><p className="mt-1 text-xl font-extrabold text-[#17233b]">{state.premium ? 'ON' : 'FREE'}</p></div><div className="min-w-[95px] border border-[#e1e4e8] bg-white px-4 py-3"><p className="text-[10px] text-[#697386]">Priority left</p><p className="mt-1 text-xl font-extrabold text-[#17233b]">{priorityRemaining}</p></div></div>
          </div>
        </section>
        <div className="space-y-5 pt-6">
          {message && <div className="flex items-center gap-2 border border-red-100 bg-[#fff5f5] px-4 py-3 text-xs font-semibold text-[#b8171d]"><CheckCircle2 className="h-4 w-4"/>{message}</div>}

          <Card className="p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><h2 className="text-sm font-extrabold text-[#17233b]">Premium Membership</h2>{state.premium && <span className="border border-red-100 bg-[#fff5f5] px-2 py-1 text-[9px] font-bold uppercase text-[#b8171d]">Active</span>}</div><p className="mt-1 text-xs text-[#697386]">₹149/month — priority matching, profile boosts, advanced discovery and priority support.</p>{state.premiumUntil && <p className="mt-2 flex items-center gap-1 text-[10px] font-semibold text-[#697386]"><Clock3 className="h-3 w-3"/> Access until {new Date(state.premiumUntil).toLocaleDateString()}</p>}</div><Button size="sm" onClick={togglePremium}>{state.premium ? 'Cancel access' : 'Activate Premium'}</Button></div>
          </Card>

          <div className="grid gap-4 md:grid-cols-2">
            {strategies.map((item) => {
              const Icon = item.icon;
              const active = item.id === 'verified' ? state.verified : item.id === 'matching' ? state.priorityMatching : item.id === 'sponsored' ? state.sponsoredEnabled : item.id === 'workshops' ? state.workshopsEnabled : item.id === 'corporate' ? state.corporateInterest : item.id === 'leads' ? state.leadGenerationEnabled : item.id === 'featured' ? Boolean(state.featuredUntil && new Date(state.featuredUntil).getTime() > Date.now()) : false;
              const action = item.id === 'featured' ? boostProfile : item.id === 'verified' ? () => patch({ verified: true, verificationRequestedAt: new Date().toISOString() }, 'Verification request submitted.') : item.id === 'matching' ? () => state.premium ? patch({ priorityMatching: !state.priorityMatching }, state.priorityMatching ? 'Priority matching disabled.' : 'Priority matching enabled.') : notify('Premium membership is required for priority matching.') : item.id === 'sponsored' ? () => patch({ sponsoredEnabled: !state.sponsoredEnabled }, 'Sponsored placement preference updated.') : item.id === 'workshops' ? () => patch({ workshopsEnabled: !state.workshopsEnabled }, 'Workshop creator preference updated.') : item.id === 'corporate' ? () => patch({ corporateInterest: true }, 'Corporate plan interest registered.') : item.id === 'leads' ? () => patch({ leadGenerationEnabled: !state.leadGenerationEnabled }, 'Lead generation preference updated.') : () => patch({ credits: state.credits + 100 }, '100 skill credits added for product testing.');
              const label = item.id === 'featured' ? (active ? 'Boost again' : 'Boost profile') : item.id === 'verified' ? (state.verified ? 'Verified' : 'Request verification') : item.id === 'credits' ? '+100 Credits' : active ? 'Disable' : 'Enable';
              return <Card key={item.id} className="p-5"><div className="flex items-start gap-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center border border-red-100 bg-[#fff5f5] text-[#d31d24]"><Icon className="h-5 w-5"/></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h3 className="text-sm font-bold text-[#17233b]">{item.title}</h3><p className="mt-0.5 text-[11px] font-semibold text-[#d31d24]">{item.value}</p></div>{active && <span className="border border-red-100 bg-[#fff5f5] px-2 py-1 text-[10px] font-bold text-[#b8171d]">Active</span>}</div><p className="mt-2 text-xs leading-relaxed text-[#697386]">{item.description}</p>{item.id === 'matching' && <p className="mt-2 text-[10px] font-semibold text-[#697386]">{priorityRemaining} priority matches remaining today.</p>}{item.id === 'verified' && state.verificationRequestedAt && !state.verified && <p className="mt-2 text-[10px] text-[#697386]">Request submitted {new Date(state.verificationRequestedAt).toLocaleDateString()}.</p>}<Button size="sm" variant="outline" className="mt-3" onClick={action}>{label}</Button></div></div></Card>;
            })}
          </div>
