      ...
      <div className="mx-auto w-full max-w-[1320px]">
        ...
        <div className="grid gap-5 pt-6 xl:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <aside className="space-y-5">
            ...
            <Card className="p-5"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#d31d24]">Outcome</p>{exchange.status === 'COMPLETED' ? <><div className="mt-3 flex items-center gap-2 text-sm font-bold text-[#17233b]"><CheckCircle2 className="h-4 w-4 text-[#d31d24]" /> Exchange completed by both participants</div><p className="mt-2 text-xs leading-5 text-[#697386]">{exchange.has_reviewed ? 'Your feedback has been recorded and contributes to community trust.' : 'Share feedback to help build a more trustworthy local exchange network.'}</p></> : <><p className="mt-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">Proposal</p><p className="mt-2 text-sm leading-6 text-[#17233b]">“{exchange.proposal_message || 'Skill exchange proposal'}”</p></>}</Card>
          </aside>
        </div>
      </div>
