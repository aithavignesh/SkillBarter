...
export const SkillMatchesPage: React.FC = () => {
  ...
  const [showFirstExchangeGuidance, setShowFirstExchangeGuidance] = useState(() => {
    return window.localStorage.getItem('skillbarter_first_exchange_guidance_seen') !== 'true';
  });
  ...
  const handleProposalSuccess = () => {
    ...
    window.localStorage.setItem('skillbarter_first_exchange_guidance_seen', 'true');
    setShowFirstExchangeGuidance(false);
    setNotice('Your first exchange is underway.');
    window.setTimeout(() => navigate('/exchanges'), 650);
  };
  ...
  return (
    ...
    >
      ...
      {showFirstExchangeGuidance && <div className="first-exchange-guidance mt-5 border border-[#ead0d1] bg-[#fff8f8] px-4 py-3" role="note"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#d31d24]">How your first exchange works</p><p className="mt-1 text-xs leading-5 text-[#17233b]">Choose a partner <span className="text-[#d31d24]">→</span> Propose the exchange <span className="text-[#d31d24]">→</span> Agree with your partner <span className="text-[#d31d24]">→</span> Start learning.</p></div><button type="button" onClick={() => { window.localStorage.setItem('skillbarter_first_exchange_guidance_seen', 'true'); setShowFirstExchangeGuidance(false); }} className="shrink-0 text-xs font-semibold text-slate-400 hover:text-[#17233b]" aria-label="Dismiss first exchange guidance">×</button></div></div>}
          ...
          {displayedMatches.map((match, idx) => {
            ...
            return (
              <article
                ...
                className={`match-person-card grid gap-5 px-5 py-5 outline-none lg:grid-cols-[minmax(220px,1fr)_minmax(260px,1.3fr)_220px] lg:items-center ${expanded ? 'match-person-card--expanded' : ''}`}
              >
                <div className="flex min-w-0 items-start gap-3">
                  <img src={match.candidate?.avatar_url || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=120&auto=format&fit=crop&q=80'} alt="" className="h-12 w-12 shrink-0 rounded-full border border-[#dfe3e8] object-cover" />
                  <div className="min-w-0">
                    <p className="break-words text-sm font-bold text-[#17233b]">{match.candidate?.full_name}</p>
                    <p className="break-words text-xs text-slate-500">{match.candidate?.headline || 'Student & Peer Learner'}</p>
                    <div className="mt-1.5 flex flex-wrap gap-2">
                      {match.candidate?.verified && <span className="inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider text-[#17233b]"><BadgeCheck className="h-3 w-3" />Verified</span>}
                      {match.candidate?.premium && <span className="inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider text-[#d31d24]"><Crown className="h-3 w-3" />Premium</span>}
                      {match.candidate?.featured && <span className="inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider text-[#d31d24]"><Rocket className="h-3 w-3" />Featured</span>}
                    </div>
                    <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-semibold">
                      <span className="flex items-center gap-1 text-slate-500"><MapPin className="h-3 w-3 text-[#d31d24]" />{match.distance_display}</span>
                      <span className="text-slate-600">★ {Math.round(match.candidate?.trust_score || 0)} trust</span>
                    </div>
                  </div>
                </div>
                <div className="min-w-0">
                  <p className="mb-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Skills to exchange</p>
                  <div className="match-exchange-panel grid min-w-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 border border-[#e1e4e8] bg-[#f7f8f7] p-3 text-xs">
                    <div className="min-w-0"><p className="text-[9px] font-bold uppercase text-slate-500">You can teach</p><p className="mt-1 break-words font-bold text-[#17233b]">{match.matched_you_offer?.[0] || offeredList[0] || 'Your skill'}</p></div>
                    <Repeat className="h-4 w-4 shrink-0 text-[#d31d24]" aria-hidden="true" />
                    <div className="min-w-0 text-right"><p className="text-[9px] font-bold uppercase text-slate-500">They can teach</p><p className="mt-1 break-words font-bold text-[#17233b]">{match.they_offer?.[0] || 'Their skill'}</p></div>
                  </div>
                  <div className={`match-person-card__detail ${expanded ? 'match-person-card__detail--visible' : ''}`} aria-hidden={!expanded}>
                    <p className="mt-3 text-[10px] font-bold uppercase tracking-wider text-[#d31d24]">Why this partner fits</p>
                    <div className="mt-2 space-y-1">{match.reasons.slice(0, 4).map((reason, index) => <p key={index} className="flex items-start gap-1.5 break-words text-[11px] text-slate-600"><CheckCircle className="mt-0.5 h-3 w-3 shrink-0 text-[#d31d24]" />{reason}</p>)}</div>
                  </div>
                </div>
                <div className="min-w-0 border-t border-[#e1e4e8] pt-4 lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0">
                  <div className="mb-3 border-b border-[#e1e4e8] pb-3 text-[10px] text-slate-600">
                    <span className="font-bold uppercase tracking-wider text-slate-500">Why connect?</span>
                    <p className="mt-1 break-words text-[#17233b]">{match.reasons?.[0] || 'Your learning goal overlaps with what this peer can teach.'}</p>
                  </div>
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <Link to={`/profile/${match.candidate?.id}`} onClick={event => event.stopPropagation()} className="min-w-0"><Button size="sm" variant="outline" className="w-full whitespace-normal leading-4">View learning profile</Button></Link>
                    <Button size="sm" className="w-full whitespace-normal leading-4" onClick={event => { event.stopPropagation(); openProposal(match); }} icon={<ArrowRight className="h-3.5 w-3.5" />}>Start First Learning Exchange</Button>
                  </div>
                </div>
              </article>
            );
