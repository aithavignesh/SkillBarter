...
export const SkillMatchesPage: React.FC = () => {
  ...
  return (
    <AppPageShell
      ...
      description="See peers whose skills align with what you want to learn — and where your skills can help them too."
    ...
    >
      ...
      <div className="mt-5 border border-[#e1e4e8] bg-white">
        ...
        {loading ? <div className="py-20 text-center text-sm text-slate-400">Calculating matches…</div> : displayedMatches.length === 0 ? <div className="py-20 text-center"><Repeat className="mx-auto h-9 w-9 text-slate-300" /><h3 className="mt-3 text-sm font-bold text-[#17233b]">No learning matches yet</h3><p className="mx-auto mt-1 max-w-md text-xs text-slate-500">Add more teaching and learning skills to discover better peer-learning matches.</p></div> : <div className="divide-y divide-[#e1e4e8]">
          {displayedMatches.map((match, idx) => <div key={idx} className="grid gap-5 px-5 py-5 lg:grid-cols-[minmax(220px,1fr)_minmax(260px,1.3fr)_220px] lg:items-center">
            ...
            <div><div className="mb-3 flex items-center justify-between"><span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Learning fit</span><span className="text-lg font-extrabold text-[#d31d24]">{match.match_score}%</span></div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 border border-[#e1e4e8] bg-[#f7f8f7] p-3 text-xs"><div><p className="text-[9px] font-bold uppercase text-slate-400">You can teach</p><p className="mt-1 truncate font-bold text-[#17233b]">{match.matched_you_offer?.[0] || offeredList[0] || 'Your skill'}</p></div><Repeat className="h-4 w-4 text-[#d31d24]" /><div className="text-right"><p className="text-[9px] font-bold uppercase text-slate-400">They can teach</p><p className="mt-1 truncate font-bold text-[#17233b]">{match.they_offer?.[0] || 'Their skill'}</p></div></div><div className="mt-3 space-y-1">{match.reasons.slice(0,4).map((r,i)=><p key={i} className="flex items-center gap-1.5 text-[11px] text-slate-500"><CheckCircle className="h-3 w-3 text-[#d31d24]" />{r}</p>)}</div></div>
            <div className="border-t border-[#e1e4e8] pt-4 lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0"><div className="mb-3 space-y-2 text-[11px] text-slate-500"><p className="flex items-center justify-between"><span>Skills</span><b className="text-slate-700">{match.score_breakdown.skill_compatibility}/50</b></p><p className="flex items-center justify-between"><span>Proximity</span><b className="text-slate-700">{match.score_breakdown.location_proximity}/20</b></p><p className="flex items-center justify-between"><span>Trust</span><b className="text-slate-700">{match.score_breakdown.trust}/20</b></p><p className="flex items-center justify-between"><span>Availability</span><b className="text-slate-700">{match.score_breakdown.availability}/10</b></p></div><div className="mb-3 border-t border-[#e1e4e8] pt-2 text-[10px] text-slate-500"><span className="font-bold uppercase tracking-wider text-slate-400">Why it fits</span><p className="mt-1 text-[#17233b]">{match.reasons?.[0] || 'Reciprocal skills and compatible exchange signals.'}</p></div><div className="grid grid-cols-2 gap-2"><Link to={`/profile/${match.candidate?.id}`}><Button size="sm" variant="outline" className="w-full">View profile</Button></Link><Button size="sm" className="w-full" onClick={() => openProposal(match)} icon={<ArrowRight className="h-3.5 w-3.5" />}>Start Exchange</Button></div></div>
          </div>)}
        </div>}
      </div>
