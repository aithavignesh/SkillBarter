...
export const TrustSystemPage: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      api.getTrustDetails(currentUser.id).then((data) => {
        setTrustData(data);
        setLoading(false);
  ...
  const score = currentUser?.trust_score || 94;

  const breakdown = [
    { title: 'Community Review Quality', weight: '40%', score: trustData?.breakdown?.review_quality || 38, max: 40, desc: 'Ratings and written feedback from verified barters' },
    { title: 'Completion Reliability', weight: '25%', score: trustData?.breakdown?.completion_reliability || 24, max: 25, desc: 'Ratio of successfully confirmed exchanges vs cancellations' },
    { title: 'Proposal Response Rate', weight: '15%', score: trustData?.breakdown?.response_rate || 14.5, max: 15, desc: 'How promptly you respond to incoming barter requests' },
    { title: 'Exchange Volume History', weight: '10%', score: trustData?.breakdown?.exchange_history || 9.5, max: 10, desc: 'Proven track record across multiple trades' },
    { title: 'Safety & Good Standing', weight: '10%', score: trustData?.breakdown?.safety_standing || 10, max: 10, desc: 'Zero upheld safety violations or misconduct reports' },
  ];
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <h2 className="text-2xl font-black text-slate-900 mt-0.5">
                {score >= 90 ? 'Exceptional Neighbor' : 'Trusted Exchanger'}
              </h2>
              <p className="text-xs text-slate-600 mt-1 max-w-md">
                Your high trust score grants your barter proposals priority visibility on local radar feeds and skill match rankings.
              </p>
            </div>
          </div>
          ...
          <div className="text-center sm:text-right border-t sm:border-t-0 sm:border-l border-slate-200/80 pt-4 sm:pt-0 sm:pl-8 shrink-0">
            <div className="text-2xl font-black text-slate-900 font-mono">
              {currentUser?.completed_exchanges_count || 18}
            </div>
