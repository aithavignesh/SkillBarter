...
export const DiscoverPage: React.FC = () => {
  ...
  return <AppPageShell eyebrow="Learning discovery" title="Find people who can teach what you want to learn" description="Discover student peers by learning goals, skills, trust and distance — then start an exchange." icon={<Compass className="h-3.5 w-3.5" />} actions={<Link to="/matches"><Button size="sm" icon={<ArrowUpRight className="h-3.5 w-3.5" />}>My learning matches</Button></Link>} search={{ value: searchQuery, onChange: setSearchQuery, placeholder: 'Search a skill you want to learn…' }}>
    ...
    {viewMode==='MAP'?<div className="border border-[#e1e4e8] bg-white p-2"><CommunityMap users={filteredNeighbors}/></div>:<div className="overflow-hidden border border-[#e1e4e8] bg-white"><div className="hidden grid-cols-[minmax(260px,1.4fr)_minmax(180px,1fr)_minmax(180px,1fr)_170px] border-b border-[#e1e4e8] bg-[#f7f8f7] px-5 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-400 md:grid"><span>Learning partner</span><span>They can teach</span><span>Trust / distance</span><span className="text-right">Action</span></div>{loading?<div className="py-20 text-center text-sm text-slate-400">Finding peers who can help you learn…</div>:filteredNeighbors.length===0?<div className="py-16 px-5 text-center"><Compass className="mx-auto h-9 w-9 text-slate-300"/><h3 className="mt-3 text-sm font-bold text-[#17233b]">No learning partners found</h3><p className="mx-auto mt-1 max-w-md text-xs leading-5 text-slate-500">No one matches these filters right now. Broaden your search or switch to My Learning Matches for complementary skill recommendations.</p><div className="mt-5 flex flex-wrap justify-center gap-2"><button onClick={()=>{setSearchQuery('');setSelectedCategory('All');setRadiusKm(100);setIntentFilter('ALL');setAvailabilityFilter('ALL');}} className="border border-[#e1e4e8] bg-white px-3 py-2 text-xs font-bold text-slate-600 hover:border-slate-300">Clear filters</button><Link to="/matches"><Button size="sm" icon={<Sparkles className="h-3.5 w-3.5" />}>View My Learning Matches</Button></Link></div></div>:<div className="divide-y divide-[#e1e4e8]">{filteredNeighbors.map(neighbor=><div key={neighbor.id} className="grid gap-4 px-5 py-4 transition hover:bg-[#fafafa] md:grid-cols-[minmax(260px,1.4fr)_minmax(180px,1fr)_minmax(180px,1fr)_170px] md:items-center"><div className="flex min-w-0 items-center gap-3"><img src={neighbor.avatar_url||'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=80'} alt={neighbor.full_name} className="h-11 w-11 shrink-0 rounded-full border border-[#dfe3e8] object-cover"/><div className="min-w-0"><p className="truncate text-sm font-bold text-[#17233b]">{neighbor.full_name}</p><p className="truncate text-xs text-slate-500">{neighbor.headline||'SkillBarter member'}</p><div className="mt-1 flex flex-wrap gap-1.5">{neighbor.verified&&<span className="text-[9px] font-bold uppercase tracking-wider text-[#17233b]">Verified</span>}{neighbor.premium&&<span className="text-[9px] font-bold uppercase tracking-wider text-[#d31d24]">Premium</span>}</div>{neighbor.featured&&<span className="mt-1 inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider text-[#d31d24]"><Rocket className="h-3 w-3"/>Featured</span>}</div></div><div className="flex flex-wrap gap-1.5">{(neighbor.skills_offered||[]).slice(0,4).map((s:string)=><span key={s} className="border border-[#e1e4e8] bg-[#f7f8f7] px-2 py-1 text-[10px] font-semibold text-slate-700">{s}</span>)}</div><div className="text-xs text-slate-500"><p className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5 text-[#d31d24]"/>{neighbor.distance_display||`${neighbor.distance_km||''} km away`}</p><p className="mt-1 font-semibold text-slate-700">★ {Math.round(neighbor.trust_score||0)} trust</p></div><div className="w-full md:w-auto md:flex md:justify-end"><Button size="sm" className="w-full md:w-auto whitespace-normal leading-4" onClick={()=>openProposal(neighbor)} icon={<Repeat className="h-3.5 w-3.5"/>}>Start Learning Exchange</Button></div></div>)}</div>}</div>}
    {selectedPartner&&<ProposeExchangeModal isOpen={isProposeOpen} onClose={()=>setIsProposeOpen(false)} partner={selectedPartner} defaultPartnerSkill={defaultPartnerSkill} defaultMySkill={defaultMySkill} onSuccess={()=>{setIsProposeOpen(false); setNotice('Learning request sent.'); navigate('/exchanges');}}/>}
  </AppPageShell>;
};