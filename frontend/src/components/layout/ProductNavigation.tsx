...
import { LayoutGrid, ChevronDown, UserRound, Compass, ArrowLeftRight, Bell, Users, GraduationCap, Coins, ShieldCheck, LifeBuoy, Sparkles } from 'lucide-react';

export const productNavigationGroups = [
  { label: 'Daily Features', icon: Sparkles, items: [['New Feature Suite','/daily-features']] },
  ...
  return (
    <div className="relative hidden md:block">
      <button type="button" onClick={() => setOpen(v => !v)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-colors ${active ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`} aria-expanded={open}>
        <LayoutGrid className="w-4 h-4" /> More <ChevronDown className={`w-3.5 h-3.5 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && <><button aria-label="Close menu" className="fixed inset-0 z-40 cursor-default" onClick={() => setOpen(false)} /><div className="absolute right-0 mt-2 z-50 w-[760px] max-w-[calc(100vw-2rem)] rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl"><div className="mb-3 flex items-center justify-between border-b border-slate-100 pb-3"><div><p className="text-sm font-bold text-slate-900">SkillBarter Workspace</p><p className="text-[11px] text-slate-500">All integrated product screens and daily improvements</p></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700">40+ features</span></div><div className="grid max-h-[70vh] grid-cols-2 gap-4 overflow-y-auto pr-1 lg:grid-cols-3">{productNavigationGroups.map(group => { const GroupIcon = group.icon; return <div key={group.label}><div className="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-slate-400"><GroupIcon className="h-3.5 w-3.5" />{group.label}</div><div className="space-y-0.5">{group.items.map(([label,path]) => <Link key={path} to={path} onClick={() => setOpen(false)} className={`block rounded-lg px-2.5 py-1.5 text-xs transition-colors ${location.pathname === path ? 'bg-emerald-50 font-semibold text-emerald-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>{label}</Link>)}</div></div>; })}</div></div></>}
    </div>
  );
};
