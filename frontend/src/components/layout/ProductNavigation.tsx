  ...
  return (
    <div className="relative w-full">
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className={`group flex w-full items-center gap-2.5 px-3 py-2.5 text-[12px] font-semibold transition-colors ${open || active ? 'text-[#d31d24]' : 'text-[#697386] hover:bg-[#fafbfc] hover:text-[#17233b]'}`}
        aria-expanded={open}
      ...
      {open && (
        <>
          ...
          <div id="skillbarter-all-features" className="absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden border border-[#dfe3e7] bg-white shadow-[0_18px_45px_rgba(23,35,59,.14)]">
            <div className="border-b border-[#edf0f2] bg-white px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[12px] font-bold text-[#17233b]">All features</p>
                  <p className="mt-0.5 text-[9px] text-[#8a92a0]">Explore every SkillBarter workspace.</p>
                </div>
              ...
              <div className="relative mt-3">
                ...
                <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search features" className="h-8 w-full border border-[#e2e5e9] bg-[#fafbfc] pl-8 pr-3 text-[10px] text-[#17233b] outline-none focus:border-[#cbd0d6] focus:bg-white" />
              </div>
            </div>

            <div className="max-h-[52vh] overflow-y-auto bg-[#fbfcfc] px-2 py-2">
              {filteredGroups.length === 0 ? (
                <div className="px-4 py-8 text-center"><Search className="mx-auto h-5 w-5 text-[#c1c6cd]" /><p className="mt-2 text-[11px] font-semibold text-[#697386]">No features found</p><p className="mt-1 text-[9px] text-[#9aa1ac]">Try another search term.</p></div>
                ...
                return (
                  <section key={group.label} className="mb-1 last:mb-0">
                    <button type="button" onClick={() => toggle(group.label)} className={`flex w-full items-center gap-2.5 px-3 py-2.5 text-left transition-colors ${groupActive ? 'bg-[#fff6f6] text-[#d31d24]' : 'bg-white text-[#536075] hover:bg-[#f6f7f8] hover:text-[#17233b]'}`} aria-expanded={expanded}>
                      ...
                      <span className="text-[9px] text-[#a0a6af]">{group.items.length}</span>
                      <ChevronDown className={`h-3 w-3 text-[#8f98a5] transition-transform ${expanded ? 'rotate-180' : ''}`} />
                    </button>
                    {expanded && (
                      <div className="border-t border-[#f0f1f3] bg-white px-2 py-1.5">
                        {group.items.map(([label, path]) => {
                          const itemActive = location.pathname === path || location.pathname.startsWith(path + '/');
                          return <Link key={path} to={path} onClick={() => { setOpen(false); setQuery(''); }} className={`flex items-center border-l-2 px-3 py-1.5 text-[10px] transition-colors ${itemActive ? 'border-[#d31d24] bg-[#fff7f7] font-semibold text-[#d31d24]' : 'border-transparent text-[#66738a] hover:border-[#d9dde2] hover:bg-[#fafbfc] hover:text-[#17233b]'}`}><span className="mr-2 h-1 w-1 bg-current opacity-50" />{label}</Link>;
                        })}
                      </div>
                    )}
                  </section>
...
export const ProductNavigationMobile: React.FC = () => {
  ...
  return (
    <div className="relative flex-1">
      <button type="button" onClick={() => setOpen(v => !v)} className={`flex w-full flex-col items-center py-1 px-1 text-[10px] font-medium ${active || open ? 'font-bold text-[#d31d24]' : 'text-[#7b8492]'}`} aria-expanded={open}>
        <LayoutGrid className="h-5 w-5" /><span className="mt-0.5">More</span>
      </button>
