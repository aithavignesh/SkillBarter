    ...
    <>
      <aside className="hidden lg:flex fixed inset-y-0 left-0 z-40 w-[312px] flex-col border-r border-[#e4e6e9] bg-white">
        ...
        <div className="border-b border-[#edf0f2] px-5 py-4">
          <Link to={`/profile/${currentUser.id}`} className="group flex items-center gap-3 py-1">
            <img src={currentUser.avatar_url || fallbackAvatar} alt={currentUser.full_name} className="h-10 w-10 rounded-full object-cover" />
        ...
        <nav className="flex-1 overflow-y-auto px-5 py-5">
          ...
          <div className="space-y-0.5">
            {primary.map(({ label, path, icon: Icon }) => {
              ...
              return (
                <Link key={path} to={path} className={`flex items-center gap-3 border-l-2 px-3 py-2.5 text-[13px] transition-colors ${active ? 'border-[#d31d24] bg-[#fff7f7] font-semibold text-[#d31d24]' : 'border-transparent text-[#596579] hover:bg-[#fafbfc] hover:text-[#17233b]'}`}>
          ...
          <div className="space-y-0.5">
            {community.map(({ label, path, icon: Icon }) => {
              const active = isActive(path);
              return (
                <Link key={path} to={path} className={`flex items-center gap-3 border-l-2 px-3 py-2.5 text-[13px] transition-colors ${active ? 'border-[#d31d24] bg-[#fff7f7] font-semibold text-[#d31d24]' : 'border-transparent text-[#596579] hover:bg-[#fafbfc] hover:text-[#17233b]'}`}>
                  <Icon className={`h-[17px] w-[17px] ${active ? 'text-[#d31d24]' : 'text-[#8a93a1]'}`} />
          ...
          <div className="mt-5 flex items-center border-l-2 border-transparent px-3 py-2">
            <ProductNavigation />
          </div>
          ...
          <div className="space-y-0.5">
            <Link to="/support/support" className={`flex items-center gap-3 border-l-2 px-3 py-2.5 text-[13px] ${isActive('/support') ? 'border-[#d31d24] bg-[#fff7f7] font-semibold text-[#d31d24]' : 'border-transparent text-[#596579] hover:bg-[#fafbfc] hover:text-[#17233b]'}`}><HelpCircle className="h-[17px] w-[17px]" />Help & Support</Link>
            <Link to="/settings/account-settings" className={`flex items-center gap-3 border-l-2 px-3 py-2.5 text-[13px] ${isActive('/settings') ? 'border-[#d31d24] bg-[#fff7f7] font-semibold text-[#d31d24]' : 'border-transparent text-[#596579] hover:bg-[#fafbfc] hover:text-[#17233b]'}`}><Settings className="h-[17px] w-[17px]" />Settings</Link>
          </div>
        </nav>
      ...
      <header className="sticky top-0 z-30 h-[68px] border-b border-[#e4e6e9] bg-white lg:ml-[312px]">
        <div className="flex h-full items-center gap-4 px-5 sm:px-7 lg:px-8">
          <div className="min-w-0 flex-1">
            <div className="hidden md:flex items-center gap-2 text-[11px] text-[#8a92a0]">
              <span>SkillBarter</span><span>/</span><span className="font-semibold text-[#17233b]">{primary.find(x => isActive(x.path))?.label || 'Community'}</span>
            </div>
          ...
          <div className="flex items-center gap-1">
            <div className="relative">
              <button type="button" aria-label="Notifications" onClick={() => { setShowNotifications(v => !v); setShowProfileMenu(false); }} className="relative flex h-9 w-9 items-center justify-center text-[#6f7887] hover:bg-[#f6f7f8] hover:text-[#17233b]">
                <Bell className="h-[18px] w-[18px]" />{unreadCount > 0 && <span className="absolute right-2 top-2 h-1.5 w-1.5 bg-[#d31d24]" />}
              </button>
              {showNotifications && <div className="absolute right-0 mt-2 w-80 border border-[#e1e4e8] bg-white shadow-[0_12px_32px_rgba(23,35,59,.12)]">
                <div className="flex items-center justify-between border-b border-[#edf0f2] px-4 py-3"><span className="text-[12px] font-bold text-[#17233b]">Notifications</span>{unreadCount > 0 && <button type="button" onClick={markAllAsRead} className="text-[10px] font-semibold text-[#d31d24]">Mark all read</button>}</div>
                <div className="max-h-80 overflow-y-auto">{notifications.length === 0 ? <div className="p-6 text-center text-[12px] text-[#8a92a0]">No notifications yet</div> : notifications.map(n => <div key={n.id} onClick={() => { markAsRead(n.id); if (n.link) { navigate(n.link); setShowNotifications(false); } }} className={`cursor-pointer border-b border-[#f0f1f3] px-4 py-3 hover:bg-[#fafbfc] ${!n.is_read ? 'bg-[#fff8f8]' : ''}`}><div className="flex items-start justify-between gap-2"><span className="text-[12px] font-semibold text-[#17233b]">{n.title}</span>{!n.is_read && <span className="mt-1 h-1.5 w-1.5 bg-[#d31d24]" />}</div><p className="mt-1 text-[11px] leading-5 text-[#697386]">{n.message}</p></div>)}</div>
              </div>}
            </div>
            ...
            <div className="relative">
              <button type="button" onClick={() => { setShowProfileMenu(v => !v); setShowNotifications(false); }} className="flex items-center gap-2 px-2 py-1.5 hover:bg-[#f6f7f8]">
                <img src={currentUser.avatar_url || fallbackAvatar} alt={currentUser.full_name} className="h-8 w-8 rounded-full object-cover" />
                <span className="hidden xl:block max-w-[120px] truncate text-[12px] font-semibold text-[#17233b]">{currentUser.full_name}</span>
                <ChevronRight className="hidden xl:block h-3.5 w-3.5 rotate-90 text-[#9aa1ac]" />
              </button>
              {showProfileMenu && <div className="absolute right-0 mt-2 w-60 border border-[#e1e4e8] bg-white shadow-[0_12px_32px_rgba(23,35,59,.12)]">
                <div className="border-b border-[#edf0f2] px-4 py-3"><p className="text-[12px] font-bold text-[#17233b]">{currentUser.full_name}</p><p className="mt-0.5 truncate text-[10px] text-[#8a92a0]">{currentUser.email}</p></div>
                <div className="py-1">
                ...
                <div className="border-t border-[#edf0f2] p-1"><button type="button" onClick={async () => { setShowProfileMenu(false); await logout(); navigate('/'); }} className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-[12px] font-semibold text-[#d31d24] hover:bg-[#fff7f7]"><LogOut className="h-4 w-4" />Log out</button></div>
              </div>}
            </div>
          </div>
        </div>
