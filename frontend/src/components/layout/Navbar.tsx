...
import { Repeat, Compass, Sparkles, ArrowLeftRight, MessageSquare, Bell, Search, User as UserIcon, ShieldCheck, LogOut, Shield, Check, Coins, Users, Settings, HelpCircle, ChevronRight } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  return <>
    <aside className="hidden lg:flex fixed inset-y-0 left-0 z-40 w-[252px] flex-col border-r border-[#e1e5ea] bg-white">
      <div className="flex h-[84px] items-center border-b border-[#edf0f3] px-7">
        <Link to="/feed" className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#d31d24] text-white shadow-[0_8px_20px_rgba(211,29,36,.18)]"><Repeat className="h-5 w-5" /></div>
          <div><div className="text-[20px] font-extrabold tracking-tight text-[#17233b]">Skill<span className="text-[#d31d24]">Barter</span></div><div className="text-[9px] font-semibold uppercase tracking-[.16em] text-[#8b95a5]">Community workspace</div></div>
        </Link>
      </div>
      <div className="border-b border-[#edf0f3] p-4">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-5">
        ...
        <div className="space-y-1">{navLinks.map(({label,path,icon:Icon}) => { const active = location.pathname === path || (path !== '/feed' && location.pathname.startsWith(path + '/')); return <Link key={path} to={path} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-semibold transition-all ${active ? 'bg-[#fff1f1] text-[#d31d24] shadow-[inset_3px_0_0_#d31d24]' : 'text-[#66738a] hover:bg-[#f6f7f9] hover:text-[#17233b]'}`}><Icon className={`h-[17px] w-[17px] ${active ? 'text-[#d31d24]' : 'text-[#7b8799]'}`} />{label}{label === 'Skill Matches' && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-[#d31d24]" />}</Link>; })}</div>
        ...
        <Link to="/support/support" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-semibold text-[#66738a] hover:bg-[#f6f7f9] hover:text-[#17233b]"><HelpCircle className="h-[17px] w-[17px] text-[#7b8799]" />Help & Support</Link>
        <Link to="/settings/account-settings" className="mt-1 flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-semibold text-[#66738a] hover:bg-[#f6f7f9] hover:text-[#17233b]"><Settings className="h-[17px] w-[17px] text-[#7b8799]" />Settings</Link>
      </nav>
      <div className="border-t border-[#edf0f3] p-4"><div className="rounded-xl bg-[#fff8f8] px-3 py-2.5"><div className="flex items-center justify-between"><span className="text-[10px] font-semibold text-[#7b8799]">Trust score</span><span className="text-xs font-extrabold text-[#d31d24]">{Math.round(currentUser.trust_score)}/100</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#f3dfe0]"><div className="h-full rounded-full bg-[#d31d24]" style={{width:`${Math.min(100, Math.max(0, currentUser.trust_score))}%`}} /></div></div></div>
    </aside>

    <header className="sticky top-0 z-30 h-[72px] border-b border-[#e1e5ea] bg-white/95 backdrop-blur-md lg:ml-[252px]">
      <div className="flex h-full items-center gap-4 px-4 sm:px-6 lg:px-8">
        <div className="min-w-0 flex-1"><div className="hidden sm:flex items-center gap-2 text-[11px] font-semibold text-[#7b8799]"><span className="rounded-full bg-[#fff1f1] px-3 py-1 font-bold text-[#d31d24]">Workspace</span><span>/</span><span className="text-[#17233b]">{navLinks.find(x => location.pathname === x.path)?.label || 'Community'}</span></div><form onSubmit={handleSearchSubmit} className="relative mt-0 sm:mt-1 max-w-xl"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8b95a5]" /><input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search skills, neighbors, or requests..." className="h-9 w-full rounded-lg border border-[#e1e5ea] bg-[#f8fafc] pl-9 pr-4 text-xs text-[#17233b] outline-none transition focus:border-[#d31d24] focus:bg-white focus:ring-2 focus:ring-[#d31d24]/10" /></form></div>
