      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <Link to={currentUser ? "/feed" : "/"} className="flex items-center gap-2.5 group">
          ...
          <div>
            <span className="text-xl font-extrabold tracking-tight bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">
              Skill<span className="text-emerald-600">Barter</span>
            </span>
            <span className="hidden sm:block text-[10px] font-medium text-slate-500 -mt-1 tracking-wider uppercase">
              Zero-Cash Community
            </span>
          </div>
        </Link>
        ...
        {currentUser && (
          <form onSubmit={handleSearchSubmit} className="hidden md:flex flex-1 max-w-xs lg:max-w-md relative">
            ...
            <input
              type="text"
              placeholder="Search skills, neighbors, or requests..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-slate-100/80 hover:bg-slate-100 text-slate-900 placeholder:text-slate-400 pl-9 pr-4 py-2 rounded-xl text-xs border border-transparent focus:border-emerald-500 focus:bg-white focus:outline-none transition-all"
            />
          </form>
        )}
        ...
        {currentUser ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/signup"
              className="text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl shadow-sm transition-all"
            >
              Join Community
            </Link>
          </div>
        ...
        {currentUser && (
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden lg:block text-left text-xs leading-tight pr-1">
                  ...
                  <p className="text-[10px] text-emerald-700 font-bold">
                    ★ {Math.round(currentUser.trust_score)} Trust
                  </p>
                </div>
              </button>
              ...
              {/* Profile Dropdown */}
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-200 py-1.5 overflow-hidden z-50">
                  <div className="px-4 py-2.5 border-b border-slate-100">
                    <p className="text-xs font-bold text-slate-900">{currentUser.full_name}</p>
                    <p className="text-[11px] text-slate-500 truncate">{currentUser.email}</p>
                    <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-emerald-700 font-semibold bg-emerald-50 px-2 py-0.5 rounded-md">
                      <ShieldCheck className="w-3.5 h-3.5" />
                      Trust Score: {Math.round(currentUser.trust_score)}/100
                    </div>
                  </div>
                  ...
                  <div className="py-1">
                    ...
                    {currentUser.is_admin && (
                      <Link
                        to="/admin"
                        onClick={() => setShowProfileMenu(false)}
                        className="flex items-center gap-2 px-4 py-2 text-xs text-amber-700 hover:bg-amber-50 font-medium border-t border-slate-100"
                      >
