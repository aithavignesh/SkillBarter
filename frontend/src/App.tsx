...
const ProtectedRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center text-xs text-slate-500" role="status">Authenticating community member…</div>;
...
const ProfileIntegratedRoute: React.FC = () => {
  const { id = '' } = useParams();
  return profileScreenIds.has(id) ? <AdditionalScreen /> : <LinkedInStyleProfilePage />;
};
...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="ricoz-theme min-h-screen min-w-0 bg-slate-50 text-slate-900">
      <Navbar />
      <main className={`min-w-0 min-h-[calc(100vh-72px)] pb-16 md:pb-0 ${appClass}`}>
        <Routes>
          ...
          <Route path="/invite" element={<ProtectedRoute><InvitePeersPage /></ProtectedRoute>} />
          <Route path="/feedback" element={<ProtectedRoute><BetaFeedbackPage /></ProtectedRoute>} />
          ...
          <Route path="/workshops/:id" element={<ProtectedRoute><WorkshopScreensPage /></ProtectedRoute>} />
